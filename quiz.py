...
def intro():
    name = input("What's your name?")
    print(f"Hi {name}!")
    print(f"You're going to play a quiz. There are 5 random trivia questions to answer, and they get progressively harder. Your score will be shown at the end. You can only answer with yes or no. Goodluck {name}")
...
while play == True:
    print(questions[0][0])
    response = input(questions[0][1])
    if answers[0] in response:
        print("Correct! The Pacific ocean is approximately 63-64 million square miles.")
        print(f"{ART_CORRECT}")
        score+=1
    
    print(questions[1][0])
    response = input(questions[1][1])
